...
const TIPS: Tip[] = [
  ...
  { keys: '📋 剪贴板中转', desc: 'claude 里 /copy 或拖选复制的内容会存到右下角 📋 列表（即使系统剪贴板被浏览器拦截也不丢），并同时尽力写入系统剪贴板——若因无用户手势被拦截，会在你下一次点击/按键时自动补写；点某条可编辑后「发送」回 claude、复制或删除' },
  { keys: '📄 文件预览', desc: '在 📋 里点开的内容若是文件路径（绝对路径，或相对 claude 当前运行目录），会自动在下方分栏只读预览文件；📄 栏 MD/原文 切换 Markdown 渲染，⛶ 放大为悬浮窗查阅、复制全文' },
  { keys: '📁 文件浏览器', desc: '右下角 📁 打开可拖拽的文件管理器，默认定位到 claude 当前目录：浏览/进入文件夹、一键复制绝对路径或发送给 claude、预览文本/Markdown、在终端 cd；并可新建文件夹/文件、重命名、删除、上传（可拖拽文件进面板）、下载。地址栏输入路径回车可直接跳转' },
  { keys: '🖼️ 粘贴/拖入图片', desc: '在终端里 Ctrl+V / Ctrl+Shift+V 粘贴图片，或直接把图片文件拖进终端：自动上传到服务器临时文件，并把该文件路径插入输入框（不自动回车）。claude 会识别 .png/.jpg/.gif/.webp 路径并作为图片读取——可再补充说明后回车发送。需 https（安全上下文）剪贴板才能读图' },
